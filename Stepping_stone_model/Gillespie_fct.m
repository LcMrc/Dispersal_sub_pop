...
function [XBlist, nfixlist, tfixlist] = Gillespie_fct(Nit, D, K, fA, gA, fB, gB, mA, mB)
    ...
    for i = 1 : Nit
        ...
        while sum(XB) ~= 0 && sum(XB) ~= D
            ...
            Tsup = 2*NB*phiB*mB;    % Transition rate associated with an increase in the number of mutant demes
            Tinf = 2*NA*phiA*mA;    % Transition rate associated with a decrease in the number of mutant demes
