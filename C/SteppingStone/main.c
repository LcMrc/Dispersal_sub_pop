...
double SumMigrationRate(double M1, int X1[], double M2, int X2[], int D)
{
	...
	S = 2*(M1*X1[0]+M2*X2[0]+M1*X1[1]+M2*X2[1]+M1*X1[2]+M2*X2[2]+M1*X1[3]+M2*X2[3]+M1*X1[4]+M2*X2[4]);
...
int main()
{
	...
	double Tvect[5], Tvectbis[5], Tvectter[20], t = 0, tlist[1000];
	...
	while (k < Nit)
	{
		...
		while (XASum != 0 && XBSum != 0)
		{
			...
			else
			{
				...
				CumSum(Tvectter, 20-1);
				ir2 = SamplingTowerLinear(Tvectter, 20);
